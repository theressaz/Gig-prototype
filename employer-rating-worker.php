...
// Available active projects list for reference
$activeContracts = [
    'CTR-GIG-2026-0811' => [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
    $overallRating = (int)($_POST['overall_rating'] ?? 5);
    if ($overallRating < 1) $overallRating = 1;
    if ($overallRating > 5) $overallRating = 5;

    $ratingQuality = (int)($_POST['rating_quality'] ?? 5);
    $ratingCommunication = (int)($_POST['rating_communication'] ?? 5);
    $ratingTimeliness = (int)($_POST['rating_timeliness'] ?? 5);
    ...
    if ($comment === '') {
        ...
        if (!isset($_SESSION['custom_reviews'])) {
            $_SESSION['custom_reviews'] = [];
        }
        if (!isset($_SESSION['completed_projects'])) {
            $_SESSION['completed_projects'] = [];
        }

        $newReview = [
            'employer' => $username,
            'project' => $projectData['title'],
            'contractId' => $projectData['id'],
            ...
            'date' => date('M Y'),
            'comment' => $comment,
            'badges' => is_array($selectedBadges) ? $selectedBadges : [],
            'recommend' => $recommend
        ];

        $_SESSION['custom_reviews'][$worker['id']][] = $newReview;
...
<?php if ($submitted): ?>
  ...
  <div class="white-card" style="text-align:center;padding:48px 24px;border:2px solid #10b981;background:linear-gradient(to bottom, #ffffff, #f0fdf4);max-width:760px;margin:0 auto;">
    <div style="width:72px;height:72px;background:#10b981;color:#ffffff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2.2rem;margin:0 auto 18px;box-shadow:0 8px 20px rgba(16,185,129,0.3);">
      ✓
    </div>
    <h2 style="font-size:1.5rem;font-weight:800;color:#065f46;margin-bottom:8px;">Proyek Berhasil Diselesaikan!</h2>
    <p style="font-size:0.95rem;color:#047857;max-width:540px;margin:0 auto 24px;line-height:1.5;">
      Terima kasih telah memberikan ulasan dan rating untuk <strong><?php echo htmlspecialchars($worker['name'], ENT_QUOTES, 'UTF-8'); ?></strong>. Ulasan Anda telah dipublikasikan dan status kontrak telah resmi diperbarui menjadi <strong>Selesai</strong>.
    </p>
    ...
    <div style="background:#ffffff;border:1px solid #bbf7d0;border-radius:12px;padding:16px 20px;max-width:500px;margin:0 auto 28px;text-align:left;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
        ...
        <span style="font-size:1.1rem;color:#f59e0b;font-weight:800;"><?php echo gig_stars($_POST['overall_rating'] ?? 5); ?> (<?php echo (int)($_POST['overall_rating'] ?? 5); ?>/5)</span>
      </div>
      <div style="font-size:0.86rem;color:var(--text-dark);font-style:italic;line-height:1.4;">
        "<?php echo htmlspecialchars((string)($_POST['comment'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"
      </div>
    </div>
    ...
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
      ...
      <a href="worker-profile.php?id=<?php echo urlencode($worker['id']); ?>&active=1" class="filter-btn-pill" style="text-decoration:none;padding:10px 20px;font-size:0.9rem;border-color:#cbd5e1;background:#ffffff;">
...
<?php else: ?>
  ...
  <div style="display:grid;grid-template-columns:1fr 340px;gap:24px;align-items:start;">
    ...
    <form method="post" action="" class="white-card" style="padding:28px;">
      <div style="padding-bottom:18px;border-bottom:1px solid var(--border-subtle);margin-bottom:24px;">
        ...
        <p style="font-size:0.84rem;color:var(--text-muted);margin-top:4px;">
          Penilaian Anda sangat berharga untuk membangun reputasi dan rekam jejak pekerja gig di ekosistem Kementerian Ketenagakerjaan.
        </p>
      </div>
      ...
      <div style="margin-bottom:28px;text-align:center;background:#f8fafc;border:1px solid var(--border-subtle);border-radius:14px;padding:24px 16px;">
        <label style="display:block;font-size:0.95rem;font-weight:800;color:var(--text-main);margin-bottom:6px;">
          Rating Keseluruhan untuk Pekerja Ini
        </label>
        <span style="font-size:0.8rem;color:var(--text-muted);display:block;margin-bottom:14px;">Klik bintang di bawah untuk memberikan skor (1 - 5)</span>
        
        <input type="hidden" name="overall_rating" id="overall_rating" value="5" />
        
        <div class="star-rating-picker" id="starContainer" style="display:inline-flex;gap:8px;font-size:2.6rem;cursor:pointer;user-select:none;color:#f59e0b;">
        ...
        <div id="ratingLabel" style="font-size:0.92rem;font-weight:700;color:#059669;margin-top:10px;">
          ★★★★★ 5.0 · Sangat Memuaskan (Luar Biasa)
        </div>
      </div>

      <!-- 2. SPECIFIC CRITERIA RATINGS -->
      <div style="margin-bottom:28px;">
        <h3 style="font-size:0.92rem;font-weight:800;color:var(--text-main);margin-bottom:14px;text-transform:uppercase;letter-spacing:0.04em;">
          Penilaian Aspek Spesifik
        </h3>
        ...
        <div style="display:flex;flex-direction:column;gap:14px;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;background:#ffffff;border:1px solid #e2e8f0;padding:12px 16px;border-radius:10px;">
            <div>
              <strong style="font-size:0.86rem;color:var(--text-main);display:block;">Kualitas Hasil Pekerjaan (Deliverables)</strong>
              <span style="font-size:0.75rem;color:var(--text-muted);">Kerapian, ketelitian, dan kesesuaian dengan brief proyek</span>
            </div>
            <select name="rating_quality" style="padding:6px 12px;border:1px solid #cbd5e1;border-radius:6px;font-size:0.84rem;font-weight:700;color:#1e293b;background:#f8fafc;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;background:#ffffff;border:1px solid #e2e8f0;padding:12px 16px;border-radius:10px;">
            <div>
              <strong style="font-size:0.86rem;color:var(--text-main);display:block;">Komunikasi &amp; Responsivitas</strong>
              <span style="font-size:0.75rem;color:var(--text-muted);">Kecepatan membalas pesan, koordinasi, dan kemudahan diajak diskusi</span>
            </div>
            <select name="rating_communication" style="padding:6px 12px;border:1px solid #cbd5e1;border-radius:6px;font-size:0.84rem;font-weight:700;color:#1e293b;background:#f8fafc;">
              <option value="5">★★★★★ 5 (Sangat Responsif)</option>
              <option value="4">★★★★☆ 4 (Responsif)</option>
              <option value="3">★★★☆☆ 3 (Cukup)</option>
              <option value="2">★★☆☆☆ 2 (Lambat)</option>
              <option value="1">★☆☆☆☆ 1 (Tidak Responsif)</option>
            </select>
          </div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;background:#ffffff;border:1px solid #e2e8f0;padding:12px 16px;border-radius:10px;">
            <div>
              <strong style="font-size:0.86rem;color:var(--text-main);display:block;">Ketepatan Waktu &amp; Deadline</strong>
              <span style="font-size:0.75rem;color:var(--text-muted);">Penyelesaian pekerjaan sesuai durasi yang disepakati</span>
            </div>
            <select name="rating_timeliness" style="padding:6px 12px;border:1px solid #cbd5e1;border-radius:6px;font-size:0.84rem;font-weight:700;color:#1e293b;background:#f8fafc;">
              <option value="5">★★★★★ 5 (Tepat Waktu / Lebih Cepat)</option>
              <option value="4">★★★★☆ 4 (Tepat Waktu)</option>
              <option value="3">★★★☆☆ 3 (Sedikit Terlambat)</option>
              <option value="2">★★☆☆☆ 2 (Terlambat)</option>
              <option value="1">★☆☆☆☆ 1 (Sangat Terlambat)</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <!-- 3. COMPLIMENT BADGES -->
      <div style="margin-bottom:28px;">
        <label style="display:block;font-size:0.92rem;font-weight:800;color:var(--text-main);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.04em;">
          Berikan Lencana Apresiasi (Opsional)
        </label>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;">
          ...
          $availableBadges = [
              ...
              '🎯 Hasil Rapi & Teliti',
              '💡 Solutif & Inisiatif Tinggi',
              '🤝 Kerjasama Luar Biasa',
              '🎨 Kreativitas Tinggi',
              '🏆 Profesional & Berintegritas'
          ];
          foreach ($availableBadges as $badge): ?>
            <label style="cursor:pointer;display:inline-flex;align-items:center;gap:6px;background:#f1f5f9;border:1px solid #cbd5e1;padding:6px 12px;border-radius:9999px;font-size:0.8rem;font-weight:600;color:#334155;transition:all 0.15s;" class="badge-checkbox-label">
              <input type="checkbox" name="badges[]" value="<?php echo htmlspecialchars($badge, ENT_QUOTES, 'UTF-8'); ?>" style="accent-color:#2563eb;" onchange="this.parentElement.style.background = this.checked ? '#eff6ff' : '#f1f5f9'; this.parentElement.style.borderColor = this.checked ? '#3b82f6' : '#cbd5e1'; this.parentElement.style.color = this.checked ? '#1d4ed8' : '#334155';" />
      ...
      <div style="margin-bottom:28px;">
        <label for="comment" style="display:block;font-size:0.92rem;font-weight:800;color:var(--text-main);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.04em;">
          Ulasan &amp; Testimoni Tertulis <span style="color:#ef4444;">*</span>
        </label>
        <textarea id="comment" name="comment" rows="4" required placeholder="Tuliskan pengalaman Anda bekerja bersama mitra ini. Contoh: Hasil desain prototype sangat rapi dan sesuai dengan design system. Tessa sangat komunikatif dan responsif dalam menerima feedback..." style="width:100%;padding:12px;border:1px solid #cbd5e1;border-radius:8px;font-size:0.88rem;line-height:1.5;outline:none;font-family:inherit;"></textarea>
      </div>
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px;margin-bottom:28px;display:flex;flex-direction:column;gap:12px;">
        <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;font-size:0.84rem;color:var(--text-dark);line-height:1.4;">
          ...
          <span><strong>Konfirmasi Deliverable Selesai:</strong> Saya menyatakan seluruh deliverable pekerjaan telah diserahkan, diuji, dan diterima dengan baik oleh perusahaan.</span>
        </label>
    ...
    <!-- SIDEBAR: PROJECT & WORKER SUMMARY -->
    <aside style="display:flex;flex-direction:column;gap:18px;">
      ...
      <div class="white-card" style="padding:20px;">
        ...
        <a href="worker-profile.php?id=<?php echo urlencode($worker['id']); ?>&active=1" target="_blank" style="font-size:0.8rem;color:var(--primary-blue);text-decoration:none;font-weight:700;display:inline-flex;align-items:center;gap:4px;">
      ...
      <div class="white-card" style="padding:20px;">
        ...
        <div style="padding-top:10px;border-top:1px solid #f1f5f9;">
          ...
          <p style="font-size:0.78rem;color:#334155;background:#f8fafc;padding:8px 10px;border-radius:6px;margin:0;">
            <?php echo htmlspecialchars($projectData['deliverables'], ENT_QUOTES, 'UTF-8'); ?>
          </p>
        </div>
      </div>
  ...
  <!-- Star Picker Interactive Script -->
  <script>
    (function() {
